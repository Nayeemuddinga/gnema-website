...
def fetch(url: str, timeout: int = 20):
    req = urllib.request.Request(url, headers={"User-Agent": "G-NeMa-OpsBot/1.0 (+https://gnema.in)"})
    try:
...
def extract_canonical(text: str):
    m = re.search(r'<link[^>]+rel=["\']canonical["\'][^>]+href=["\']([^"\']+)["\']', text, flags=re.I)
...
def has_jsonld(text: str):
    return bool(re.search(r'<script[^>]+type=["\']application/ld\+json["\']', text, flags=re.I))
...
def audit_html(config):
    ...
    for path in sorted(ROOT.rglob("*.html")):
        ...
        if title and len(title) > 70:
            warnings.append(f"title longer than 70 characters ({len(title)} characters)")
        description = extract_meta(text, "description")
        if description and len(description) > 170:
            warnings.append(f"description is long ({len(description)} characters)")
...
def audit_sitemap(config):
    ...
    if not sitemap.exists():
        return {"ok": False, "message": "sitemap.xml missing", "urls": []}
...
def audit_robots(config):
    ...
    if not robots.exists():
        return {"ok": False, "message": "robots.txt missing"}
...
def ranking_check(config):
    ...
    if not api_key:
        return {"enabled": True, "available": False, "message": "SERPAPI_KEY not set; ranking check skipped.", "results": []}
    results = []
    for query in ranking.get("queries", []):
        ...
        if status != 200:
            results.append({"query": query, "status": "error", "position": None})
            continue
        try:
            data = json.loads(body)
        except Exception:
            results.append({"query": query, "status": "invalid-response", "position": None})
            continue
        position = None
        matched_url = None
...
def create_report(config, html_results, sitemap, robots, rankings):
    ...
    for item in html_results:
        ...
        lines.append(f"| `{item['file']}` | {len(item['errors'])} | {len(item['warnings'])} | `{canonical}` | |")
    ...
    if not rankings.get("available"):
        ...
        lines.extend(["| Query | Position | URL |", "|---|---:|---|"])
        for result in rankings.get("results", []):
            position_text = str(result.get("position")) if result.get("position") else "Not found"
            lines.append(f"| {result['query']} | {position_text} | {result.get('url') or '-'} |")
    lines.extend(["", "## Recommended Actions", "", "1. Review any metadata errors.", "2. Keep sitemap URLs aligned with canonical URLs.", "3. Inspect important pages in Google Search Console after deployment.", "4. Expand content around validated search-intent topics.", "5. Review ranking movement over time rather than reacting to one measurement.", "", "G-NeMa Ops Bot does not automatically send outreach messages."])
    return "\n".join(lines) + "\n"
...
def main():
    ...
    if error_count:
        print(f"OPS RESULT: FAIL — {error_count} metadata error(s).", file=sys.stderr)
        return 1
    if not sitemap.get("ok"):
        print("OPS RESULT: FAIL — sitemap validation failed.", file=sys.stderr)
        return 1
    if not robots.get("ok"):
        print("OPS RESULT: FAIL — robots validation failed.", file=sys.stderr)
        return 1
